%{
    ...
    map<string, Node_ptr> FunctionTable;                 // register named-function, store FUN_EXP node ptr 
...
%token<str> id
%token<bval> bool_val
%token<op> Plus Minus Mul Div Mod Greater Smaller Equal And Or Not
...
%type <node> IF_EXP TEST_EXP THEN_ELSE_EXP
...
LOGICAL_OP      :   '(' And EXP EXPS ')'        { $$ = newNode($3, $4, "And", 1); }
                |   '(' Or EXP EXPS ')'         { $$ = newNode($3, $4, "Or", 1); }		// if at least one exp is true return true, otherwise return false
                |   '(' Not EXP ')'             { $$ = newNode($3, NULL, "Not", 1); } 	// if exp false return true, otherwise return false
                ;

IF_EXP          :   '(' IF TEST_EXP THEN_ELSE_EXP ')'   { $$ = newNode($3, $4, "IF", 0);}
                ;
TEST_EXP        :   EXP                                 { $$ = $1; }					// if true return then, false return else
                ;
THEN_ELSE_EXP   :   EXP EXP                             { $$ = newNode($1, $2, "THEN_ELSE_EXP"); }
                ;
...
ARGUMENTS       :   EXP ARGUMENTS               { $$ = newNode($1, $2, "ARGUMENTS", 4);}			//do it while noNameFun
                |   EXP                         { $$ = newNode($1, NULL, "ARGUMENTS", 4); }
                |                               { $$ = newNode(NULL, NULL, "ARGUMENTS", 4); }
                ;
PARAMETERS      :   VARIABLE PARAMETERS         { $$ = newNode($1, $2, "PARAMETERS", 4);}			//do it when define a named function
...
Node* newNode(Node *l, Node *r, string type, int rtype, int ival, string name, bool bval) {
    ...
    n->right = r;
    n->type = type;
    n->rtype = rtype;					// return type, 0 as NUM, 1 as LOGIC, 2 as ID, 3 as FUN, 4 as Pars or Args
    n->ival = ival;
    n->name = name;
    n->bval = bval;
    return n;
}
Var* newVar(int rtype, int ival, bool bval) {// store the variable
...
void traverseAST(Node *node) {
    ...
    // do action based on node's type
        ...
        if(FunctionMode == "none") {					 
            it = varTable.find(node->name); 			// find in table
            if(it != varTable.end()){					// if find
                node->rtype = it->second.rtype;
                node->ival = it->second.ival;			// give iv& bv to node
                node->bval = it->second.bval;
            }
        } else {
            if(FunctionMode != "noNameFun")                  // if is no-named func, traverse FUN first
                traverseAST(FunctionTable[node->name]);
            it3 = parTable[FunctionMode].find(node->name);	 // find in table
            if(it3 != parTable[FunctionMode].end()){
                node->rtype = it3->second->rtype;			 // give iv& bv to node
    ...
    //  ***************************** DEFINE *****************************
        ...
        if(node->right->rtype == 0 || node->right->rtype == 1) {
            ...
            if(cvDA(node->name, 1)){
                ...
                varTable[node->name].ival = (node->right->rtype == 0)? node->right->ival : 0;    // store in map
                varTable[node->name].bval = (node->right->rtype == 1)? node->right->bval : 0;  
            } else {
                cout << "You can't redefining exist variable !!" << endl;
            }
        }
        // define a function name : store function ptr in FunctionTable 
        else if(node->right->rtype == 3){
            
            node->name = node->left->name;
            node->rtype = node->right->rtype;
            if(cvDA(node->name, 2)){
                FunctionTable[node->name] = node->right;  
            } else {
    ...
    //  ***************************** IF_EXP *****************************
        ...
        traverseAST(node->left);
        // if TEST_EXP = TRUE -> do THAN-EXP(left child of THEN_ELSE_EXP node)
        if(node->left->bval){
        ...
        // if TEST_EXP = FALSE -> do ELSE-EXP(right child of THEN_ELSE_EXP node)
        else{
    ...
    //  ***************************** FUN *****************************
        ...
        traverseAST(node->left);                // traverse parameters first, create parTable for <FunctionMode>
        bAtP(FunctionMode);                 // node(id) will find value in parameter
        ...
        traverseAST(node->right);               // trace arguments first
        FunctionMode = node->left->name;
        traverseAST(node->left);                // then trace variable(FUN-NAME) -> trace FUN
        // get fun result
        node->rtype = FunctionTable[FunctionMode]->rtype;
        node->ival  = FunctionTable[FunctionMode]->ival;
        node->bval  = FunctionTable[FunctionMode]->bval;
        // reset
        ...
        if(node->right != NULL && node->right->type != "PARAMETERS")
            parTable[FunctionMode][node->right->name] = newVar(0, 0, 0);
        if(node->left != NULL && node->left->type != "PARAMETERS")
            parTable[FunctionMode][node->left->name] = newVar(0, 0, 0);
    } 
    else if(node->type == "ARGUMENTS") {			// store the variable that add in to function
        traverseAST(node->left);
        traverseAST(node->right);
        if(node->right != NULL && node->right->type != "ARGUMENTS") {
            if(node->right->rtype == 0) {
                args.push_back(newVar(0, node->right->ival, 0));    //Push one var as arg
            }else if(node->right->rtype == 1) {
                args.push_back(newVar(1, 0, node->right->bval));	//Push one var as arg
            } 
        }
        if(node->left != NULL && node->left->type != "ARGUMENTS") {
            if(node->left->rtype == 0) {
                args.push_back(newVar(0, node->left->ival, 0));		//Push one var as arg
            }else if(node->left->rtype == 1) {
                args.push_back(newVar(1, 0, node->left->bval));		//Push one var as arg
            } 
        }
...
// ***************************** Support function *****************************
int num_op_action(string type){// return result in EXPS for operator : Plus Mul Equal
    int res = (type == "Mul")? 1 : 0;  // ets * 0 = 0, so initialize it
    if(type == "Equal")
        res = num_action[0];
...
bool bool_op_action(string type){// return result in EXPS for operator : And Or
    bool res = (type == "And")? 1 : 0;    // like mult => mult initial value
...
bool cvDA(string name, int mode) {// to check if this variable is defined already, mode 1 for variable name, 2 for function name
    if(mode == 1) {
        it = varTable.find(name);		//find name
        if(it == varTable.end())		//haven't find til map end    
            return true;				//define it
    } else {
        it4 = FunctionTable.find(name);
        if(it4 == FunctionTable.end())
            return true;
    }
    return false;
}
void bAtP(string funName) {// bind arguments to parameters for function
